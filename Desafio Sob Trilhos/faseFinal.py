...
def show_quiz_screen():
    ...
    try:
        # Tenta carregar a imagem do quiz de forma mais robusta
        BASE_DIR = os.path.dirname(__file__)
        IMG_DIR = os.path.join(BASE_DIR, "imgs")
        background = pygame.image.load(os.path.join(IMG_DIR, "quizcerto.png")).convert()
        background = pygame.transform.scale(background, (800, 600))
    except Exception as e:
        print(f"Erro ao carregar imagem do quiz: {e}")
        background = pygame.Surface((800, 600))
        background.fill((0, 0, 0))

    # Loop principal da tela do quiz
    while True:
...
def fase_final(player_name=None, tempo_acumulado=0):
    ...
    # Definiindo os obstáculos na tela
    obstaculos = [
    ...
    while running:
        ...
        # Salva a posição anterior do protagonista
        ...
        # Verifica se o jogador alcançou a área de vitória
        if prota.colliderect(ganhar_area):
            ...
            # Mostra a tela do quiz e espera pelo ENTER
            if show_quiz_screen():
